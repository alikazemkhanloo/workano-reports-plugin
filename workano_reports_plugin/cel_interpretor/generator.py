...
from .dao import get_context_numbers, get_schedule_from_extension, get_schedule_from_outcall, get_schedule_from_path, get_schedule_from_exten_tenant
from .schedule_utils import get_schedule_mapper
...
class _ParticipantsProcessor:
    ...
    def _compute_participants_from_channels(
    ...
    ) -> list[ReportsCallLogParticipant]:
        ...
        for channel_name, raw_attributes in call_log.raw_participants.items():
            ...
            participant = ReportsCallLogParticipant(user_uuid=confd_participant.uuid)
    ...
    def _fetch_participants(self, call_log: RawCallLog):
        ...
        for user_uuid in user_uuids:
            ...
            if not user_connected_participants:
                ...
                if not confd_participant:
                    continue
                participant = ReportsCallLogParticipant(
...
class CallLogsGenerator:
    ...
    def call_logs_from_cel(self, cels: list[CEL]) -> list[ReportsCallLog]:
        result = []
    ...
    def _check_schedule(self, call_log: RawCallLog):
        ...
        if schedule:
            ...
            call_log.schedule_state = {
                ...
                'schedule_id': getattr(schedule_model, 'id', None),
                'schedule_name': getattr(schedule_model, 'name', None),
